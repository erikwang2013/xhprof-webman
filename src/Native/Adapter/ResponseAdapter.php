...
/**
 * 原生 PHP 响应适配器：不持有任何框架对象，`send()` 直接向 SAPI 输出。
 *
 * 原生应用没有「框架接管返回值」这一层（没有 PSR-7、没有 Response 对象），
 * 所以与 WordPress 适配器同形：`withX()` 攒状态、`send()` 一次吐出
 * 状态行（http_response_code）+ 头（header）+ 体（echo）。
 *
...
class ResponseAdapter implements ResponseInterface
{
    ...
    {
        // 与 Hyperf / WordPress 适配器同形：读不出文件就退化成 404，MIME 由 StaticController 推断。
