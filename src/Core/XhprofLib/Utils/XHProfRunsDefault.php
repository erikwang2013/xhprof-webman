...
class XHProfRunsDefault implements XHProfRuns
{
    ...
    public static function get_run($run_id, $type, &$run_desc)
    {
        ...
        $res = Xhprof::$cache->get(Xhprof::$key_prefix . ':xhprof_log:' . $run_id);
    ...
    protected static function _checkLogNum()
    {
        ...
        $num = Xhprof::$cache->incr(Xhprof::$key_prefix . ":run_id_num");
        if ($num > Xhprof::$log_num) {
            $old_run_id = Xhprof::$cache->rPop(Xhprof::$key_prefix . ':run_id');
            if ($old_run_id !== null) {
                Xhprof::$cache->del(
                    Xhprof::$key_prefix . ':request_log:' . $old_run_id,
                    Xhprof::$key_prefix . ':xhprof_log:' . $old_run_id
                );
                Xhprof::$cache->decr(Xhprof::$key_prefix . ':run_id_num');  //计数-1
            }
        }
        return true;
    }
    ...
    protected static function _saveToRedis($xhprof_data)
    {
        ...
        $run_id = uniqid();
        Xhprof::$cache->lPush(Xhprof::$key_prefix . ":run_id", $run_id);
        $wt = 0;   //请求总耗时
        $mu = 0;   //总消耗内存
        ...
        $method = Xhprof::$request->method();
        $http = Xhprof::$request->header('x-forwarded-proto');
        $http = !empty($http) ? $http . "://" : "";
        $row = array(
            'request_uri' => $http . Xhprof::$request->host() . Xhprof::$request->uri(),
            'method'      => $method,
            'wt'          => $wt,
            'mu'          => $mu,
        ...
        Xhprof::$cache->set($key, json_encode($row));
        $key = Xhprof::$key_prefix . ':xhprof_log:' . $run_id;   //列表存储log
        $xhprof_data_str = serialize($xhprof_data);
        if (!empty($xhprof_data_str)) Xhprof::$cache->set($key, $xhprof_data_str);
        return $run_id;
    }
    ...
    public static function list_runs2()
    {
        ...
        //取所有请求数据
        $run_id_lists = Xhprof::$cache->lRange(Xhprof::$key_prefix . ':run_id', 0, Xhprof::$log_num);
        foreach ($run_id_lists as $run_id) {
            $res = Xhprof::$cache->get(Xhprof::$key_prefix . ":request_log:" . $run_id);
            if (!$res) continue;
            ...
            $echo_page .= '<li><small class="small_filemtime">'
                ...
                . '</small><small class="small_wt ' . $wtClass . '">' . $request_arr['wt'] . '</small></small><small class="small_wt">' . $request_arr['mu'] . '</small><small class="small_log"><a href="' . htmlentities(Xhprof::$request->url())
