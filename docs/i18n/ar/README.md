> **ترجمة آلية غير مُراجَعة.** هذا المستند تُرجم آليًا ولم يُراجعه متحدث أصلي بالعربية، وقد يحمل أخطاء في المعنى والمصطلحات. المرجع المعتمد هو [النسخة الإنجليزية](../en/README.md).

[中文](../../../README.md) · [English](../en/README.md) · [한국어](../ko/README.md) · [Русский](../ru/README.md) · [Deutsch](../de/README.md) · [Français](../fr/README.md) · [Español](../es/README.md) · [Português](../pt/README.md) · **العربية** · [हिन्दी](../hi/README.md) · [বাংলা](../bn/README.md) · [Bahasa Indonesia](../id/README.md) · [日本語](../ja/README.md)
...
| PHP الخام (بدون إطار) | — (بلا حزمة خارجية) | 8.0 | `Native\XhprofBootstrap` | سطر واحد في أعلى ملف الدخول: `XhprofBootstrap::start()`، بلا تسجيل متحكم أو مسار |

تقع فئات المدخل جميعها تحت بادئة مساحة الأسماء `ErikWang2013\Xhprof\`‎ (المحذوفة من الجدول أعلاه).  ولا يحتاج أيٌّ من الأحد عشر إلى تسجيل متحكم أو مسار: فصفحة التقرير والموارد الساكنة يتولّاها صنف الدخول بنفسه (وفي حالة Drupal يتولّاها مسار الوحدة).
...
**2. صفحة التقرير والموارد الساكنة** — Drupal هي **الإطار الوحيد من الأطر الأحد عشر الذي يخدم صفحة التقرير عبر مسارات الوحدة**: فملف `xhprof.routing.yml` يسجّل مسار التقرير `/xhprof` ومسار الموارد `/xhprof-assets`، ويعرضهما متحكم الوحدة افتراضيًا؛ أما الأطر العشرة الأخرى فتخدم صفحة التقرير والموارد الساكنة بنفسها قبل بدء التنميط ولا تسجّل أي مسار. **ومع بادئة `assets_url` مخصّصة يتولّى الوسيط خدمة الموارد**: فمسار موارد الوحدة مكتوب في `xhprof.routing.yml` (`/xhprof-assets/{file}`) ولا يطابق بادئة أخرى.
...
مناسب للتطبيقات بلا إطار، التي لها متحكم أمامي واحد فقط (`public/index.php` أو ما شابه).

**1. أضف سطرًا واحدًا في أعلى ملف الدخول**:

```php
\ErikWang2013\Xhprof\Native\XhprofBootstrap::start();
```

لتغيير الإعدادات مرّر المصفوفة إلى هذا السطر (مجموعة المفاتيح نفسها لدى العشرة الآخرين، والقيم الافتراضية في `src/Native/config/xhprof.php`):
...
المعاملان الثاني والثالث نقطتا حقن اختياريتان: `CacheInterface $cache` و`LoggerInterface $logger` (الافتراضي: محوّل Redis في هذه الحزمة و`error_log`). القيمة المُعادة هي نسخة الدخول لهذا الطلب (`stop()` عديمة الأثر عند التكرار)؛ نادِها لإيقاف التنميط مبكرًا في العملية نفسها.

**2. صفحة التقرير والموارد الساكنة** — **لا حاجة إلى تسجيل متحكم أو مسار**: هذا السطر يفحص مسار الطلب قبل بدء التنميط: عند مطابقة مسار التقرير `/xhprof` يعيد صفحة التقرير مباشرةً (مع `Content-Type: text/html; charset=UTF-8` و`Cache-Control: no-cache, private`، و`auth_token` يعمل كما هو)، وعند مطابقة مسار الموارد (البادئة تُقرأ من الخيار `assets_url`، والافتراضية `/xhprof-assets`) يعيد المورد الساكن مباشرةً. **والكلفة مكتوبة صراحةً**: بعد معالجة هذين المسارين ينفّذ `exit` — فلا تُنفَّذ بقية مسار الطلب (المسارات بعد هذا السطر، وتهيئة الحاوية، وبدء الجلسة، ومنطق الإنهاء الذي يسجّله التطبيق نفسه).

**3. نافذة التنميط = هذا السطر → إغلاق العملية** (المُسجَّل هو `register_shutdown_function`). **والحدود كما هي**: لا تشمل الكود **قبل** هذا السطر (composer autoload، تهيئة المتحكم الأمامي)، ولا ما تفعله عملية أو إضافة أخرى (تحليل الطلب في php-fpm، والمعالجة في nginx). النهاية الطبيعية و`exit` وError أو الاستثناء غير الملتقَط تصل كلها إلى نقطة الإيقاف؛ أما `SIGKILL` / OOM killer فلا — تختفي حالة التنميط مع العملية ولا تبقى للطلب التالي. ولتضييق النطاق استخدم الخيار `ignore_url_arr` (مطابقة جزئية على `uri()`، ويعمل بلا تعديل كود).

**4. شغّله فعلاً بـ `php -S`**:

```sh
# في أعلى public/index.php سطر XhprofBootstrap::start()، وهو المتحكم الأمامي نفسه
php -S 127.0.0.1:8000 -t public public/index.php
```

زُر `http://127.0.0.1:8000/` لتوليد بيانات، ثم `http://127.0.0.1:8000/xhprof` لصفحة التقرير — كلاهما في العملية نفسها، وتُتحقَّق الموارد كذلك.
...
المخطط الأول هو **البنية**: فئة مدخل كل إطار من الأطر الأحد عشر، والعقود الخمسة، وطبقات النواة الثلاث، والاقترانان الوحيدان الباقيان.
...
| PHP الخام (بدون إطار) | سطر واحد `XhprofBootstrap::start()` في أعلى ملف الدخول | إغلاق العملية (`register_shutdown_function`)، مع `stop()` لإيقاف أبكر |
...
│   ├── Native/                   # PHP خام (بدون إطار): فئة الدخول و5 محوّلات
...
| الأطر الأحد عشر كلها تتشارك مجموعة مفاتيح إعدادات واحدة | اختبار تكافؤ الإعدادات (مجموعات المفاتيح لا التطابق الحرفي؛ فالتعليقات قد تختلف) |
