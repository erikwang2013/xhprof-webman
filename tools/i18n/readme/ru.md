...
| Чистый PHP (без фреймворка) | — (внешнего пакета нет) | 8.0 | `Native\XhprofBootstrap` | Одна строка в начале файла входа, `XhprofBootstrap::start()`, без контроллера и маршрутов |

Все классы входа лежат под префиксом пространства имён `ErikWang2013\Xhprof\` (в таблице он опущен).  Ни одному из одиннадцати не нужно, чтобы вы регистрировали контроллер или маршрут: страницу отчёта и статические ресурсы отдаёт сам класс входа (в случае Drupal — маршрут модуля).
...
**2. Страница отчёта и статические ресурсы** — Drupal это **единственный из одиннадцати фреймворков, идущий путём «модуль + маршруты»**: `xhprof.routing.yml` регистрирует путь отчёта `/xhprof` и путь ресурсов `/xhprof-assets`, по умолчанию их отдаёт контроллер модуля; остальные десять классов входа сами замыкают эти пути до старта профилирования и отдают страницу отчёта и статические ресурсы, не регистрируя маршрутов. **При своём префиксе `assets_url` ресурсы берёт на себя middleware**: path маршрута ресурсов модуля жёстко прописан в `xhprof.routing.yml` (`/xhprof-assets/{file}`) и никогда не совпадёт с другим префиксом.
...
Для приложений без фреймворка, где есть только один фронт-контроллер (`public/index.php` и подобные).

**1. Добавьте одну строку в начало файла входа**:

```php
\ErikWang2013\Xhprof\Native\XhprofBootstrap::start();
```

Чтобы изменить конфигурацию, передайте массив прямо в эту строку (набор ключей тот же, что у остальных десяти; значения по умолчанию — в `src/Native/config/xhprof.php`):
...
Второй и третий аргументы — необязательные точки внедрения: `CacheInterface $cache` и `LoggerInterface $logger` (по умолчанию — Redis-адаптер этого пакета и `error_log`). Возвращается экземпляр входа для текущего запроса (`stop()` идемпотентен); вызовите его, чтобы остановить профилирование раньше в том же процессе.

**2. Страница отчёта и статические ресурсы** — **контроллер и маршруты не нужны**: эта строка до старта профилирования смотрит на путь запроса: попадание в путь отчёта `/xhprof` сразу отдаёт страницу отчёта (с `Content-Type: text/html; charset=UTF-8` и `Cache-Control: no-cache, private`, `auth_token` действует как обычно), попадание в путь ресурсов (префикс читается из параметра `assets_url`, по умолчанию `/xhprof-assets`) сразу отдаёт статический ресурс. **Цена названа прямо**: обработав эти два пути, строка делает `exit` — остальная часть запроса (маршруты после этой строки, инициализация контейнера, старт сессии и собственная логика завершения, зарегистрированная приложением) не выполняется.

**3. Окно профилирования = эта строка → завершение процесса** (регистрируется `register_shutdown_function`). **Границы описаны как есть**: в него не входит код **до** этой строки (composer autoload, инициализация фронт-контроллера) и то, что делают другие процессы или расширения (разбор запроса в php-fpm, обработка на стороне nginx). Нормальное завершение, `exit`, непойманные Error / исключения доходят до точки остановки; `SIGKILL` / OOM killer — нет: состояние профилирования исчезает вместе с процессом и не остаётся следующему запросу. Сузить область помогает параметр `ignore_url_arr` (совпадение подстроки по `uri()`, действует без правки кода).

**4. Запустите по-настоящему один раз через `php -S`**:

```sh
# В начале public/index.php стоит XhprofBootstrap::start(), и этот файл — фронт-контроллер
php -S 127.0.0.1:8000 -t public public/index.php
```

Откройте `http://127.0.0.1:8000/`, чтобы получить данные, затем `http://127.0.0.1:8000/xhprof` — страницу отчёта; и то и другое в одном процессе, и ресурсы проверяются заодно.
...
Первая диаграмма — это **структура**: класс входа каждого из одиннадцати фреймворков, 5 контрактов, три слоя Core и те самые две оставшиеся связи.
...
| Чистый PHP (без фреймворка) | Одна строка `XhprofBootstrap::start()` в начале файла входа | Завершение процесса (`register_shutdown_function`), плюс `stop()` для досрочной остановки |
...
│   ├── Native/                   # Чистый PHP (без фреймворка): класс входа и 5 адаптеров
...
| Все одиннадцать фреймворков используют один набор ключей конфигурации | тест паритета конфигурации (наборы ключей, не побайтово; комментарии могут отличаться) |
...
**Смоук для чистого PHP**: запустите встроенный сервер командой `php -S 127.0.0.1:8000 -t public public/index.php` (шаг 4 в «Чистом PHP») и выполните три шага — страница отчёта и ресурсы находятся в **одном процессе** с рабочими запросами, поэтому шаг 3 проверяется сразу.

**Известное ограничение: у отображаемого в списке `request_uri` нет порта**

Контракт `host()` означает «только хост, без порта» (R-2), и все одиннадцать фреймворков его соблюдают — различается лишь реализация: `getHost()` из PSR-7 порт не несёт, Joomla / WordPress срезают его вручную через `parse_url`, а Webman и ThinkPHP требуют строгий аргумент `host(true)` (по умолчанию возвращается заголовок `Host` как есть, вместе с портом). Отображаемый в списке `request_uri` собирается как `host() . uri()` (`src/Core/XhprofLib/Utils/XHProfRunsDefault.php`), поэтому на нестандартном порту (например, `:8080`) **текст** этой строки порт не показывает. **На сами ссылки это не влияет**: ссылки в списке и в отчёте строит `XhprofLib::report_url()` — это относительные URL (только путь + query), они открывают нужную страницу и от `host()` не зависят.

**`assets_url` теперь поддерживает произвольный префикс**

Префикс статических ресурсов больше не жёстко прописанная константа: `src/Core/StaticController.php` сопоставляет пути ресурсов со значением параметра `assets_url` (по умолчанию `/xhprof-assets`, завершающий слэш необязателен). Остальное ограничение при развёртывании в подкаталоге описано ниже, в пункте про Drupal. **Все одиннадцать фреймворков следуют этому параметру**: десять классов входа сами замыкают путь ресурсов до старта профилирования и отдают их, а Drupal отдаёт префикс по умолчанию через маршрут модуля + контроллер, а свой префикс передаёт middleware. **Граница**: Laravel, Hyperf, Webman и ThinkPHP больше не нуждаются в контроллере и маршрутах — middleware выполняется первым, поэтому контроллер и два маршрута, зарегистрированные по старым инструкциям, лишь затенены: они не дают ошибки и больше никогда не достигаются.
