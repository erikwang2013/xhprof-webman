...
![Журнал запросов](docs/images/runs-list.png)

**Отчёт по одному запуску**

![Отчёт по одному запуску](docs/images/run-report.png)
...
Подмассив `redis` — особенность Yii3: когда `CacheInterface` не внедрён, middleware работает с phpredis напрямую через него. Оставьте `assets_url` может быть любым префиксом: ссылки CSS/JS на странице отчёта и `StaticController` читают один и тот же параметр (по умолчанию `/xhprof-assets`). Остальные ограничения при развёртывании в подкаталоге — в разделе [Проверка и известные ограничения](#проверка-и-известные-ограничения).
...
**Переключатель языка на странице отчёта**

Раскрывающийся список справа в навигации перечисляет 13 языков их **собственными названиями** (значение `_meta.name` из каждого словаря, например 한국어, 日本語). Ссылка каждого пункта строится из **строки запроса текущей страницы** (`XhprofLib::report_url()`), поэтому `?token=`, сортировка, `run` и все остальные параметры едут вместе с ней; переключение языка **не покидает текущее представление** — сменив язык в отчёте по запуску, вы остаётесь на том же запуске.

**Диагностика на странице отчёта**

Самая верхняя карточка в теле отчёта — «Диагностика» (сразу под описанием запуска): сначала «Почему медленно» (не более 3 причин), затем «Прочие наблюдения» (не более 3 проверок). Ссылка «подробнее» после каждого вывода ведёт на страницу метода; у рекурсии (R4) ссылка есть только тогда, когда «голое» имя действительно присутствует в таблице символов — xhprof разворачивает рекурсию в `fib@1`/`fib@2`, и если остались только развёрнутые имена, страница метода ищет `fib` и не находит ничего. Шесть правил и их пороги:

- **R1** собственное время ≥ 10% общего времени запроса;
- **R2** число вызовов ≥ 1000;
- **R3** число вызовов одной связи ≥ 500, а собственное время вызываемой функции ≥ 5% общего времени запроса;
- **R4** один и тот же символ встречается на ≥ 2 разных глубинах (рекурсия);
- **R5** собственный пик памяти ≥ 30% общего пика;
- **R6** собственное время > полного времени (`excl_wt > wt`, логически невозможно) — проба целостности данных, на здоровых данных не срабатывает.

Пороги зашиты в константы `src/Core/Analysis/Analyzer.php`, и **ни один параметр конфигурации** пока не может их изменить или отключить блок (`enable` выключает сбор, поэтому диагностировать нечего). Блок показывается **только в представлении одного запуска верхнего уровня**: ни в diff-представлении, ни на странице метода он не рендерится — передаваемые туда `$symbol_tab`/`$totals` не относятся к одному запуску (в diff-режиме это приращения run2 − run1).
...
![Обоснование проектирования](docs/images/design.svg)
...
├── <КлассВхода>.php
└── config/xhprof.php             # те же 10 ключей конфигурации, что и в остальных фреймворках
```
...
**Известное ограничение: у отображаемого в списке `request_uri` нет порта**

Контракт `host()` означает «только хост, без порта» (R-2), и все десять фреймворков его соблюдают — различается лишь реализация: `getHost()` из PSR-7 порт не несёт, Joomla / WordPress срезают его вручную через `parse_url`, а Webman и ThinkPHP требуют строгий аргумент `host(true)` (по умолчанию возвращается заголовок `Host` как есть, вместе с портом). Отображаемый в списке `request_uri` собирается как `host() . uri()` (`src/Core/XhprofLib/Utils/XHProfRunsDefault.php`), поэтому на нестандартном порту (например, `:8080`) **текст** этой строки порт не показывает. **На сами ссылки это не влияет**: ссылки в списке и в отчёте строит `XhprofLib::report_url()` — это относительные URL (только путь + query), они открывают нужную страницу и от `host()` не зависят.
...
Пакет распространяется по лицензии MIT (см. `LICENSE`); `src/Core/XhprofLib/**`, `src/html/js/xhprof_report.js` и `src/html/css/xhprof.css` — производные от [phacility/xhprof](https://github.com/phacility/xhprof) (Apache-2.0) и остаются на его условиях; список сторонних фронтенд-библиотек — в `NOTICE`.
...
Этот плагин опирается на [phacility/xhprof](https://github.com/phacility/xhprof) и [xiexianbo123/xhprof](https://github.com/xiexianbo123/xhprof).
