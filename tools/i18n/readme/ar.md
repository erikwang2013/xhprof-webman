...
المصفوفة الفرعية `redis` خاصة بـ Yii3: فعندما لا يُحقن أي `CacheInterface` يستخدمها الوسيط للتواصل مع phpredis مباشرة. وأبقِ `assets_url` على قيمته الافتراضية — فأي بادئة أخرى لا تعطي إلا استجابة 200 بجسم فارغ (فهي ثابت مكتوب في النواة؛ انظر [التحقق والقيود المعروفة](#التحقق-والقيود-المعروفة)).
...
**قيد معروف: `host()`‎ بلا منفذ**

يقصد بالعقد `host()`‎ «المضيف فقط، بلا منفذ»، لكن روابط القائمة في التقرير تُبنى على الشكل `host() . uri()`‎ (`src/Core/XhprofLib/Utils/XHProfRunsDefault.php`). لذا **على منفذ غير قياسي (مثل `:8080`) تفقد روابط القائمة المنفذ ولا تؤدي إلى شيء**. وهذه مشكلة كامنة في التنفيذ القائم (تصيب webman / Laravel / ThinkPHP / Hyperf بالقدر نفسه)، ولم تُصلَح هنا، وهي مسجّلة كقيد معروف.

**قيد معروف: `assets_url` لا يعمل إلا إذا كان `/xhprof-assets`**

بادئة الموارد ثابت مكتوب في `src/Core/StaticController.php` (`private const URI_PREFIX = '/xhprof-assets'`)، بينما تقرأ روابط CSS/JS في صفحة التقرير خيار الإعدادات `assets_url` (`src/Core/Xhprof.php`). وبمجرد أن يختلف الاثنان تعيد `getPathFromRequest()`‎ القيمة `null` وتعيد `serve()`‎ القيمة `withBody('')->withHeaders([])`‎ — **أي استجابة 200 بجسم فارغ، لا 404**. والنتيجة: اضبط `assets_url` على أي قيمة أخرى فيصمت CSS/JS ويفرغ، وتبقى صفحة التقرير بلا أنماط دون أي خطأ من أي نوع. وبعبارة أخرى، `assets_url` حاليًا خيار وهمي لا يعمل إلا عند تركه على قيمته الافتراضية. وهذه مشكلة سابقة قائمة ولم تُصلَح هنا.

**قيد معروف: حارس المسار يفشل عندما يكون Drupal في مجلد فرعي**

عند تثبيت Drupal تحت مجلد فرعي (مثل `/sites/app/xhprof`)، لا يستطيع حارس المسار مطابقة URI يحمل مسار الأساس، فيرتد السلوك إلى «يُنمَّط لكن لا يُحفظ» (ومع الإعدادات الافتراضية يعالجه `ignore_url_arr`). وهذا من الصنف نفسه الذي تنتمي إليه بادئة `assets_url` المكتوبة مباشرة.
