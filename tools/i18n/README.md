...
`I18N_DOCS = ['architecture', 'lifecycle', 'design']` (`lib.php`) is a **fixed list, not a
glob** over `docs/images/`. That directory also holds `pet.svg` — the project's pet, also
served as the report page's site icon and brand icon from `src/html/pet.svg`. It contains
no `<text>` nodes on purpose, so there is nothing in it to translate and it never enters
this pipeline: no template, no glossary key, no output under `docs/i18n/`. Only the three
diagrams are extracted and translated.
...
`docs/images/`, `README.md` or the hand-written `readme/*.md` sources.
...
* `en` is the source language of this repository (`tools/i18n/readme/en.md`),
  not a translation, and must not carry a notice.
...
English locale is generated by the same autofit, and **8 of its runs are already
...
translation of the English source `tools/i18n/readme/en.md` at
`tools/i18n/readme/<lang>.md` and let
...
`README.md`, the sibling `en/README.md` and `docs/*.png`, so a root outside the repo cannot
...
`en` is the source language, and its artefacts are not all the same kind of thing:
...
* `docs/i18n/en/README.md` is a normal output of the same generator, and it is
  ...
  language — `en` renders `tools/i18n/readme/en.md`, which is already English.
* The English entry in the switcher points at this copy, exactly as every other
  locale's entry points at its own: since the English README moved under `docs/`,
  `en` is an ordinary language entry, `tools/i18n/readme/en.md` is its body
  source, and the root `README.md` is the only hand-written README left — which
  is why the parity test checks that one switcher by hand and the other twelve
  against the language table.
...
linking every README in the family — the Chinese original at the repo root plus
the twelve locales beside it, `en` included:
